...
CRITICAL INSTRUCTION FOR "EXACT": Your EXACT output MUST be a strict, raw, literal phonetic transcription of the audio. YOU MUST NOT ADD ANY WORDS OF YOUR OWN. YOU MUST NOT CORRECT GRAMMAR. Write EXCLUSIVELY and EXACTLY the phonetic sounds the speaker makes as you hear them in the audio, mapped to Roman script. Apart from the specific spelling dictionary below, do NOT alter the spoken words. Do NOT translate, do NOT correct, and do NOT "Urdu-ify" the words. If they say "krdain", do NOT write "kar rahe", write exactly "krdain". Do not leak standard Urdu or any inferred words into the EXACT output. Transcribe ONLY what is physically heard in the voice.

Apply these specific spelling rules ONLY to your EXACT transcription (Hazara Hindko), DO NOT apply them to the ROMAN translation (Roman Urdu): where there is "vaddi" change it to "baddi", where there is "vekhde" change it to "dekhde", where there is "jeda" change it to "jerha", where there is "chhod" change it to "chorh", where there is "jane" change it to "julden", where there is "vi" change it to "b", where there is "rehnen" change it to "rehden", where there is "ajda" change it to "ajra", where there is "ana" change it to "arna", where there is "ji" change it to "g", where there is "gande" change it to "kendey", where there is "duji" change it to "doi", where there is "bhan" change it to "phan", where there is "apni" change it to "apri", where there is "ghat" change it to "khat", where there is "sakdi" change it to "akdi", where there is "wich" change it to "bich", where there is "tabla" change it to "tavla", where there is "booda" change it to "boota", where there is "baqt" change it to "vaqt", where there is "ghr" change it to "kaar", where there is "trup" change it to "trut", where there is "ajle" change it to "ajulai", where there is "sakda" change it to "akda", where there is "parhaya" change it to "parhalya", and where there is "mut" change it to "much".
